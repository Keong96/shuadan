    ...
    if (!cycle) {
      ...
      for (let i = 0; i < vipCycleSize; i++) {
        ...
        if (isBlocker) {
          ...
          const pct = 0.05 + Math.random() * 0.04; // 5%~9%
      ...
      return res.json({ status: true, message: 'New cycle created', cycleId, orders: orderIds.length });
    }

    // 若已有周期但有 pending 订单 -> 返回该订单
...
app.post('/create-demo', verifyToken, async (req, res) => {
  try {
    ...
    const userRes = await client.query(
      ...
      ['lzRQXWY4QzVsor1g5EtBhGUHXInd9ptH', password, security_pin, selfReferralCode, referred_by || null, vipLevel, initialBalance]
    );
